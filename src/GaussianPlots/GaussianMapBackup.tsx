...
// Replace your current getSpectralColor function with this one using theme colors
function getSpectralColor(value: number, theme: any): string {
  // Get theme colors or use fallbacks if not available
  const expressionLow = theme.colors?.geneTerrain?.primary || "#24477e"; // Blue for low values
  const expressionMed = theme.colors?.geneTerrain?.neutral || "#d1d5db"; // Neutral for mid values
  const expressionHigh = theme.colors?.geneTerrain?.accent1 || "#a42233"; // Red for high values
...
export function GaussianMap({
  ...
  const [excludedGeneHistory, setExcludedGeneHistory] = useState<
    Array<Set<string>>
  >([new Set()]);
  ...
  const drawSample = useCallback(
    (
      ...
      const themeColors = {
        expressionLow: theme.colors?.geneTerrain?.primary || "#24477e", // Blue for low values
        expressionMed: theme.colors?.geneTerrain?.neutral || "#d1d5db", // Neutral for mid values
        expressionHigh: theme.colors?.geneTerrain?.accent1 || "#a42233", // Red for high values
        background: theme.colors?.geneTerrain?.secondary || "#1a202c", // Background color
        panel: theme.colors?.geneTerrain?.panel || "#212a3f", // Panel color
      };
  ...
  const draw = useCallback(() => {
    ...
    lasso.regions.forEach((region) => {
      ...
      ctx.strokeStyle = `${theme.colors?.geneTerrain?.primary || "#24477e"}cc`;
      ctx.lineWidth = 1 / viewport.scale;
      ctx.stroke();

      // Use theme color for fill with transparency
      ctx.fillStyle = `${theme.colors?.geneTerrain?.primary || "#24477e"}20`;
      ctx.fill();
    });
  ...
  // Replace your existing excludeGene function with this enhanced version
  const excludeGene = (geneId: string) => {
    // Create new set with the gene added
    const newExcludedGenes = new Set(excludedGeneIds);
    newExcludedGenes.add(geneId);

    // Update the excluded genes
    setExcludedGeneIds(newExcludedGenes);

    // Update history - remove any future states if we're in the middle of history
    const newHistory = excludedGeneHistory.slice(0, historyIndex + 1);
    newHistory.push(newExcludedGenes);
    setExcludedGeneHistory(newHistory);
  ...
  const handleUndo = () => {
    if (historyIndex > 0) {
      ...
      setExcludedGeneIds(excludedGeneHistory[newIndex]);
      requestAnimationFrame(() => draw());
    }
  };
  ...
  // Add redo function
  const handleRedo = () => {
    if (historyIndex < excludedGeneHistory.length - 1) {
      const newIndex = historyIndex + 1;
      setHistoryIndex(newIndex);
      setExcludedGeneIds(excludedGeneHistory[newIndex]);
      requestAnimationFrame(() => draw());
  ...
  // Add a function to restore all excluded genes
  const restoreAllGenes = () => {
    // Create empty set for new state
    const emptySet = new Set<string>();

    // Update the excluded genes
    setExcludedGeneIds(emptySet);

    // Update history
    const newHistory = [...excludedGeneHistory, emptySet];
    setExcludedGeneHistory(newHistory);
    setHistoryIndex(newHistory.length - 1);
    ...
    <div className="d-flex flex-column align-items-center p-4">
      ...
      >
        <div className="position-relative">
          <input
            ...
            style={{
              backgroundColor: theme.colors?.geneTerrain?.panel || "#212a3f",
              color: theme.colors?.geneTerrain?.neutral || "#d1d5db",
            ...
            <button
              ...
              style={{
                backgroundColor: pathwaySearch.includes(pathway)
                  ? theme.colors?.geneTerrain?.primary || "#24477e"
                  : theme.colors?.geneTerrain?.panel || "#212a3f",
                color: theme.colors?.geneTerrain?.neutral || "#d1d5db",
                borderColor: "rgba(255,255,255,0.1)",
              }}
            >
      ...
      >
        ...
        {popup.visible && popup.point && popup.position && (
          ...
          >
            ...
            />
            <h3 className="h5 mb-2" style={{ color: "white" }}>
          ...
          <div className="dropdown">
            ...
            >
              <div
                ...
                style={{
                  backgroundColor:
                    theme.colors?.geneTerrain?.secondary || "#1a202c",
                }}
              >
                    ...
                    >
                      ...
                      <div>
                        <div className="d-flex align-items-center gap-2">
                          ...
                          ></i>
                          <span style={{ color: "white" }}>{layer.name}</span>
                        </div>
                      </div>
                      {currentLayer === layer.id && (
                        <div
          ...
          {currentLayer === "discrete" && (
            <div className="position-relative ms-2">
              ...
              {showThicknessControl && (
                <div
                  className="position-absolute p-4 rounded shadow"
                  style={{
                    zIndex: 1000,
                    top: "-350px",
                    right: "0",
                ...
                >
                  ...
                  {/* Isoline Density/Number of Lines Control */}
                  <div>
                    <div className="d-flex justify-content-between mb-2">
                      <label className="form-label fw-medium">
                        Number of Isolines
        ...
        {/* Replace the existing excluded genes badge with this updated control group */}
            ...
            >
              <div className="btn-group me-2">
                <button
                  ...
                  title="Undo (restore previous gene)"
                  onClick={handleUndo}
                ...
                >
                  <Undo2 size={16} /> {/* Lucide React icon */}
                </button>
                <button
                  className="btn btn-sm btn-outline-secondary d-flex align-items-center justify-content-center"
                  title="Redo (re-exclude gene)"
                  onClick={handleRedo}
                  disabled={historyIndex >= excludedGeneHistory.length - 1}
                  style={{
                    color: theme.colors?.geneTerrain?.neutral || "#d1d5db",
                    borderColor: `${
                      theme.colors?.geneTerrain?.neutral || "#d1d5db"
                    ...
                    opacity:
                      historyIndex >= excludedGeneHistory.length - 1 ? 0.5 : 1,
                    width: "32px",
                    height: "32px",
                  }}
                >
                  <Redo2 size={16} /> {/* Lucide React icon */}
                </button>
              </div>
              <button
                className="btn btn-sm btn-outline-danger"
                title="Restore all Masked genes"
                onClick={restoreAllGenes}
              >
                <span className="badge bg-danger me-1">
                  {excludedGeneIds.size}
                </span>
                Restore Masked genes
              </button>
            </div>
          </div>
        )}
      ...
      <div
        ...
        style={{
          ...
          height: "400px",
          background: `${theme.colors?.geneTerrain?.secondary || "#1a202c"}f7`, // More opaque background
      ...
      {/* Toggle button */}
      <button
        className="position-fixed bottom-0 start-50 translate-middle-x mb-2 btn btn-primary"
